...
def eth1_with_static_ip(eth1_up):
    ...
    apply_with_description(
        "Configure the eth1 interface with the static IPv4 address 192.0.2.251/24 and static IPv6 address 2001:db8:1::1/64",
        desired_state,
    )
    yield desired_state
...
def test_enable_mptcp_flags_and_remove(eth1_with_static_ip, mptcp_flags):
    ...
    apply_with_description(
        "Configure eth1 to have the mptcp address flag {mptcp_flags}".format(
            mptcp_flags
        ),
        desired_state,
    )
