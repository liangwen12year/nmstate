...
def restore_hostname():
    ...
    apply_with_description(
        "Set the hostname to {}".format(cur_hostname_conf),
...
def test_hostname_set_different_running_and_config(restore_hostname):
    apply_with_description(
        "Set the running hostname to nmstate-test1.example.org and set the config hostname to nmstate-test2.example.org"
    )
    libnmstate.apply(
        {
            HostNameState.KEY: {
