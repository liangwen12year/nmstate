...
def dummy_interface(name):
    ...
    apply_with_description(
        "Bring up the interface {} but disable IPv4 and IPv6".format(name),
        dummy_desired_state,
    )
    try:
        ...
        apply_with_description(
            "Delete the interface {}".format(name),
            dummy_desired_state,
        )
...
def test_iface_description_removal(eth1_up):
    ...
    apply_with_description("Empty the interface description", desired_state)
    current_state = statelib.show_only(("eth1",))
