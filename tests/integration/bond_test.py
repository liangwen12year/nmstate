...
def test_add_and_remove_bond_with_two_port(eth1_up, eth2_up):
    ...
    apply_with_description(
        "Add bond interface bond99 with two ports eth1 and eth2, set bonding mode balance-rr",
        state,
    )
...
def test_remove_bond_with_minimum_desired_state(eth1_up, eth2_up):
    ...
    apply_with_description(
        "Add bond interface bond99 with bonding mode balance-rr, attach two ports eth1 and eth2",
        state,
    )
...
def test_add_and_remove_bond_with_port_config(eth1_up, eth2_up):
    ...
    try:
        apply_with_description(
            "Add bond interface bond99 with bonding mode active-backup, attach two ports eth1 and eth2 with ports config",
            state,
        )
        assertlib.assert_state_match(state)
    finally:
...
def test_add_bond_with_port_config_and_modify(eth1_up, eth2_up):
    ...
    try:
        ...
        apply_with_description(
            "Set the ports config, set eth1 priority to -1 and queue id to 1, set eth2 priority to 9 and queue id to 0",
            state,
        )
        assertlib.assert_state_match(state)
    finally:
...
def test_add_bond_with_port_and_ipv4(eth1_up, eth2_up, setup_remove_bond99):
    ...
    apply_with_description(
        "Add the bond interface bond99, configure the static IP 192.168.122.250 with prefix length 24 for bond99, set the bonding mode to balance-rr, attach eth1 and eth2 as the ports, set the bond options miimon 140",
        desired_bond_state,
    )
...
def test_set_bond_mac_address(eth1_up):
    ...
    with bond_interface(name=BOND99, port=[port_name]) as state:
        ...
        apply_with_description(
            "Set the MAC address of the bond interface bond99 to be 02:FF:FF:FF:FF:00",
            state,
        )
        ...
        apply_with_description(
            "Set the MAC address of the bond interface bond99 to be 02:FF:FF:FF:FF:01",
            state,
        )
...
def test_adding_a_port_keeps_mac_of_existing_bond(bond99_with_eth2, eth1_up):
    ...
    apply_with_description("Add eth1 to bond99", desired_state)
...
def test_adding_port_to_empty_bond_doesnt_keep_mac(eth1_up):
    with bond_interface(BOND99, []) as state:
        ...
        current_state = statelib.show_only((bond_state[Interface.NAME],))
        apply_with_description("Add eth1 as the port for bond99", state)
        modified_state = statelib.show_only((bond_state[Interface.NAME],))
        assert (
            modified_state[Interface.KEY][0][Interface.MAC]
            != current_state[Interface.KEY][0][Interface.MAC]
...
def test_removing_port_keeps_mac_of_existing_bond(bond99_with_2_port, eth1_up):
    ...
    current_state = statelib.show_only((bond_state[Interface.NAME],))
    apply_with_description(
        "Add eth1 as the port to the bond bond99", desired_state
    )
    modified_state = statelib.show_only((bond_state[Interface.NAME],))
    assert (
        modified_state[Interface.KEY][0][Interface.MAC]
...
def test_change_bond_option_with_an_id_value(bond99_with_eth2):
    ...
    apply_with_description(
        "Set the xmit_hash_policy to layer2+3 and bonding mode to balance-xor for bond99",
        desired_state,
    )
...
def test_change_2_port_bond_mode_from_1_to_5():
    with bond_interface(
        ...
        apply_with_description(
            "Set the bonding mode to balance-tlb for bond device bond99", state
        )
...
def test_bond_disable_arp_interval(bond99_with_2_port_and_arp_monitor):
    ...
    apply_with_description("Disable arp interval on bond99", state)
...
def bond99_with_ports_and_vlans(bond99_with_2_port):
    ...
    apply_with_description(
        "Create the vlan with vlan ID 200 and base interface bond99",
...
def test_reset_bond_options_back_to_default(bond99_with_2_port):
    ...
    apply_with_description("Double the default miimon value for bond99", state)
    state = statelib.show_only((BOND99,))
    assert (
        state[Interface.KEY][0][Bond.CONFIG_SUBTREE][Bond.OPTIONS_SUBTREE][
            "miimon"
...
def test_bond_enable_and_disable_accept_all_mac_addresses(bond88_with_port):
    ...
    apply_with_description(
        "Enable accepting all mac address for bond88", desired_state
    )
    assertlib.assert_state_match(desired_state)

    desired_state[Interface.KEY][0][Interface.ACCEPT_ALL_MAC_ADDRESSES] = False
    apply_with_description(
        "Disable accepting all mac address for bond88", desired_state
    )
    assertlib.assert_state_match(desired_state)
...
def test_bond_flip_tlb_dynamic_lbs(bond99_with_2_port):
    ...
    apply_with_description(
        "Set the bonding mode to balance-tlb and enable tlb_dynamic_lb for bond99",
        desired_state,
    )
    assertlib.assert_state_match(desired_state)
...
def test_bond_preserve_existing_all_slaves_active_setting(bond99_with_2_port):
    ...
    apply_with_description(
        "Set the bonding mode to balance-tlb and all_slaves_active to dropped for bond99",
        desired_state,
    )
    assertlib.assert_state_match(desired_state)
    ...
    apply_with_description(
        "Set the tlb_dynamic_lb disabled for bond99", desired_state
    )
    assertlib.assert_state_match(desired_state)
    current_state = statelib.show_only((BOND99,))
    assert (
...
- name: eth2
  state: down""",
    ...
    apply_with_description(
        "Remove bond device bond99, bring down port eth1 and eth2",
...
def test_remove_bond_and_assign_ip_to_bond_port(bond99_with_2_port):
    ...
    apply_with_description(
        "Remove bond99 and assign IP to eth1", desired_state
    )
    assertlib.assert_state_match(desired_state)
