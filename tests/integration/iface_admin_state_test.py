...
def test_set_a_down_iface_down(eth1_up):
    ...
    apply_with_description(
        "Set down the ethernet interface eth1", desired_state
    )
    assertlib.assert_state(desired_state)
...
def test_removing_a_non_removable_iface(eth1_up):
    ...
    apply_with_description("Delete the eth1 ethernet interface", desired_state)
...
def test_change_iface_without_type(eth1_up):
    ...
    apply_with_description(
        "Change the mtu of the ethernet device eth1 to 1400", desired_state
    )
